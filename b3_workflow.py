...
class AgenticCommerceWorkflow(Workflow):
    ...
    @step
    async def route_intent(self, ctx: Context, ev: StartEvent) -> IntentClassifiedEvent | StopEvent:
        ...
        if any(w in lower_msg for w in ["buy", "checkout", "purchase", "place order", "order now", "get me"]):
            # Extract budget / max price if mentioned (e.g. "under 5000" or "under ₹8000")
            ...
            # Detect SKU or product category
            target_product = None
            ...
            # If user mentions adding wrist rest or accepting upsell
            if "wrist rest" in lower_msg or "bundle" in lower_msg or "both" in lower_msg or "add it" in lower_msg or "yes" in lower_msg:
            ...
            # Pass directly through Deterministic Guardrail Layer
            order_res = money_tools.create_order_guarded(
                cart=cart,
                user_id=user_id,
                idempotency_key=idempotency_key or f"idem_{uuid.uuid4().hex[:8]}"
            ...
            if order_res["success"]:
                ...
                # Bounded denial handled gracefully!
                return StopEvent(result={
    ...
    async def handle_catalog_search(self, ctx: Context, ev: CatalogSearchEvent) -> StopEvent:
        ...
        # Calculate dynamic upsell opportunity (Merchant Growth Engine)
        bundle_offer = read_tools.calculate_upsell_bundle(top_choice.id)

        response_text = f"I found {len(products)} matching option(s). Top recommendation: **{top_choice.name}** at ₹{top_choice.price:,.2f} (Rating: {top_choice.rating}⭐)."
        
        if bundle_offer:
            response_text += f"\n\n💡 **AI Revenue Booster Opportunity**: Bundle with {bundle_offer.complementary_product_name} for just ₹{bundle_offer.discounted_bundle_price:,.2f} (Save ₹{bundle_offer.savings_amount:,.2f} with 5% bundle discount!)."
...
print("Commerce Workflow written successfully!")
