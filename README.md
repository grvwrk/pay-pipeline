...
the system writes transaction states and audit trails to SQLite. since render's default storage is ephemeral, configure a persistent mount to prevent data loss:
1. go to the **disks** settings in your web service dashboard.
2. click **add disk**.
3. configure settings:
   - **name**: `pay-data`
   - **mount path**: `/var/data`
   - **size**: `1 GiB`
4. in the **env** settings tab, add:
   - **`DATABASE_URL`**: `sqlite:////var/data/pay_pipeline.db`
...
- **`AUDIT_HMAC_SECRET`**: `your_audit_hashchain_hmac_secret`

### 4. update webhook in razorpay dashboard
update the webhook url in your razorpay settings to point to your live deployment url:
- **url**: `https://<your-render-subdomain>.onrender.com/api/v1/webhooks/razorpay`
- **secret**: matches the value of `RAZORPAY_WEBHOOK_SECRET` set in render.